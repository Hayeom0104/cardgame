...
class Settings:
    ...
    central_base_url: str = os.environ.get("DECKOUT_CENTRAL_URL", os.environ.get("CENTRAL_BASE_URL", "http://localhost:8000"))
    central_api_key: str = os.environ.get("DECKOUT_API_KEY", os.environ.get("DECKOUT_CENTRAL_API_KEY", ""))
    #: 연동 가이드(2026-09-11) §4B — Central이 `/event`, `/shutdown`에 실어
    #: 보내는 `X-ARI-Minigame-Secret` 헤더와 상수 시간 비교할 공유 비밀.
    #: 미설정 시 기본적으로 기동을 차단한다 — 운영 배포에서는
    ...
    allow_unauthenticated_local: bool = os.environ.get("DECKOUT_ALLOW_UNAUTHENTICATED_LOCAL") == "1"
