...
| 담당 | 이 세션 | **jun9991** |
| 기준 문서 | **v6.4** | **v1** |
| 관리자 대시보드 | 전체 구현 (`app/admin/` — 로그인, 밸런스 편집, 콘텐츠 편집, 버전 초안/검증/발행/폐기, 에셋 업로드, 플레이어 조회) | 전체 구현 |

**`cardgamebot/` 쪽은 jun9991 님이 담당한다. 건드리지 않는다.** 그 브랜치의
코드를 읽는 것은 괜찮지만(참고·비교 목적), 수정하거나 이쪽으로 끌어오는 것은
오너 지시가 있을 때만 한다.
